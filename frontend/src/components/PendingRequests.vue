<template>
    <div>
      <h1 class="text-white">All Pending Requests</h1>
      <br />
      ...
      <div class="col-md-3" v-for="category in pendingCategories" :key="category.category_id">
        <div class="card mb-4" style="width: 18rem">
          ...
          <div class="card-body">
            ...
            <div class="d-flex justify-content-between card-footer">
              <a @click="approveCategory(category.category_id)" class="btn btn-success text-white px-4"
                >Accept</a
              >
              <a @click="declineCategory(category.category_id)" class="btn btn-danger text-white px-4"
                >Reject</a
              >
            </div>
          </div>
      ...
      async approveCategory(categoryId) {
        try {
          ...
          if (response.ok) {
            ...
            this.pendingCategories = this.pendingCategories.filter((category) => category.categoryId !== categoryId)
            window.location.reload()
