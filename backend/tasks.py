...
def setup_periodic_tasks(sender, **kwargs):
    ...
    sender.add_periodic_task(
        # crontab(day_of_month=30, hour=23, minute=59),
...
def send_email():
    ...
    managers = requests.get("http://127.0.0.1:5000/api/managers")
    if (managers):
        managers_list = managers.json()
    else:
        managers_list = ['menonaishwarya1101@gmail.com']

    response = requests.get("http://127.0.0.1:5000/api/generate_monthly_report")
    if (response): 

        sender = '21f1005945@ds.study.iitm.ac.in'
        # receiver = 'menonaishwarya1101@gmail.com'
        receivers = ', '.join(managers_list)
...
def export_product_data_to_csv(file_path):
    ...
    SQLALCHEMY_DATABASE_URI = os.path.join(SQLITE_DB_DIR, "store.db")
    ...
    cursor.execute(
        "SELECT p.product_id, c.category_name, p.product_name, p.price, p.avg_review FROM product p JOIN category c ON p.category_id = c.category_id")
    product_data = cursor.fetchall()

    # Define CSV header
    header = ['Product ID', 'Category Name', 'Product Name', 'Price', 'Rating']
    ...
    conn.close()
    return "Product Details CSV File Updated!"
